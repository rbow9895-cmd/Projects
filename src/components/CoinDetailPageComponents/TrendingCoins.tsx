...
const TrendingCoins = () => {
  ...
  return (
    <div className={"relative top-6 sm:mr-4"}> {   /* agar mobile pe center nahi hora toh remove this margin right  */}
      <p className="justify-center text-center text-4xl text-white font-medium mb-8 relative -top-2 interFont">Top crypto coins updates </p>
        ...
        {topCoins.length > 0 ?
          <AliceCarousel autoPlay={true} autoPlayInterval={1050} animationDuration={800} mouseTracking infinite={true} disableButtonsControls={true} disableDotsControls={true} responsive={responsive}>
            {topCoins.map((item, index) => {
              return (
                <div key={item.id + index} className="boxshDark w-80 h-44 rounded-2xl flex flex-col">
                  ...
                  <div className="flex text-sm items-center justify-between pl-3 pr-8 mb-3">
                    <p className='mt-2 text-center'><span className='text-white font-semibold'>price:</span> <span className='text-white'>$ {item.current_price.toLocaleString()}</span>  </p>
