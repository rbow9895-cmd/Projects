...
const HomeComnponent = () => {
    ...
    return (
        ...
        <div>
            ...
            {  /* SEARCH BAR */}
            <p className="justify-center text-center text-4xl text-white font-medium mb-8 relative -top-2 interFont">Market </p>
            ...
            <div className={"flex flex-wrap items-center justify-center"}>
                ...
                <div className='flex items-center gap-8 tableHeaderChild1 md:justify-center'>
                    ...
                    <div onClick={currencyModalToggle} className=" items-center gap-2 hidden lg:flex cursor-pointer border rounded w-64 justify-center h-10 text-white">
                        <p>Currency</p>
                    ...
                    <div className={"  w38rem hidden lg:block"}>
                        {currencyModal && <div className={styles.currencyModalHold}>
                            <div>
                                ...
                                <div className=' flex font-semibold gap-3 items-center cursor-pointer mb-7 text-white'>
                                    <AiOutlineDollarCircle className='text-xl font-semibold' />
                                    <p onClick={() => setCurrency("usd")}> USD </p>
                                </div>

                                <div className=' flex font-semibold gap-3 items-center cursor-pointer text-white'>
                ...
                <div className='flex gap-8 flex-wrap items-center ml-2 md:justify-center justify-around mt-5 md:mt-0'>
                    <p className={`${tabState === "all coins" ? "text-white flex justify-center items-center w-24 h-9 rounded-md border border-black cursor-pointer interFont scalee" : "flex justify-center items-center w-24 h-9 rounded-md greyBackground cursor-pointer interFont scalee"}`} onClick={allCoinsHandler}> All Coins </p>
                    <p className={`${tabState === "metaverse" ? "text-white flex justify-center items-center w-24 h-9 rounded-md border border-black cursor-pointer interFont scalee" : "flex justify-center items-center w-24 h-9 rounded-md greyBackground cursor-pointer interFont scalee"}`} onClick={metaVerseCategrotyHandler}> Metaverse </p>
                    ...
                    <p className='text-white flex justify-center items-center w-32 h-9 rounded-md greyBackground cursor-pointer interFont scalee gap-2' onClick={onOpen}>  <AiOutlineStar className='text-xl' /> Favourites </p>
                </div>
                ...
                {cryptoData.length === 0 ?
                    ...
                    <table className="table cryptoDataMainTable mx-auto w-full">
                        ...
                        <thead className='rounded-lg'>
                            <tr>
                                <th className='maincolor'></th>
                                <th className='metaverse-table maincolor text-white'>Currency</th>
                                <th className='maincolor'>
                                    <Tooltip label='Low To High' className='md:mr-20'>
                                        <span id='lowTOhigh' className='flex items-center gap-1 cursor-pointer text-white' onClick={lowTOhighHanler}>Price
                                            <HiOutlineArrowsUpDown className='text-xl' />
                                        </span>
                                    </Tooltip>
                                </th>
                                <th className='maincolor text-white'>Change</th>

                                <th className='maincolor text-white'>total volume</th>

                                <th className='maincolor'>
                                    <Tooltip label='High To Low' className='md:mr-28'>
                                        <span id='highTOlow' className='flex items-center gap-1 cursor-pointer text-white' onClick={highTOlowHandler}>Market Cap
                                ...
                                <tr key={coin.id} className='border'>
                                    <th className='maincolor'>
                                        <div className='w-8 h-8 cursor-pointer'>
                                            <AiOutlineStar className='sm:text-xl text-xl text-white hover:text-yellow-500 transition-all ease-in'
                                                onClick={() => addToFavouritesHandler(coin)}
                                            />
                                        </div>
                                    </th>
                                    <td className='py-6 cursor-pointer maincolor text-white'>
                                        <Link href={`/${coin.id}`}>
                                    ...
                                    <td className='maincolor font-semibold py-6 sm:text-base text-sm text-white'> {currency == "usd" ? "$" : <BiRupee className=' inline-block'></BiRupee>} {coin.current_price.toLocaleString()}</td>
                                    <td className={coin.price_change_percentage_24h > 0 ? "maincolor text-green-600 sm:text-base text-sm font-semibold py-10 inline-block align-middle border-transparent" : "maincolor text-red-600 sm:text-base text-sm font-semibold py-10 inline-block align-middle border-transparent"}> <span className=' inline-block align-middle'> {coin.price_change_percentage_24h > 0 ? <FiTrendingUp className='text-semibold text-xl text-green-600' /> : <FiTrendingDown className='text-semibold text-xl text-red-600' />} </span>  {coin.price_change_percentage_24h.toFixed(2)}%</td>
                                    <td className='maincolor font-semibold pl-6 sm:text-base text-sm text-white'>{coin.total_volume.toLocaleString()}</td>
                                    <td className='maincolor font-semibold text-gray-500 py-6 sm:text-base text-sm text-white'> {currency == "usd" ? "$" : <BiRupee className=' inline-block'></BiRupee>} {coin.market_cap.toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
